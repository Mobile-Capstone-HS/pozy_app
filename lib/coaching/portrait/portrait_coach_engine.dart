...
class PortraitCoachEngine {
      ...
      case LightingCondition.side:
        ...
        if (yaw > 30) {
          return CoachingResult(
            ...
            confidence: 0.7,
            reason: s.isFrontCamera
                ? '얼굴을 카메라 쪽으로 조금만 돌려 그림자를 줄여보세요'
                : '피사체 얼굴이 카메라 쪽으로 조금 더 보이게 해보세요',
          );
        }
        return null;
  ...
  CoachingResult? _evaluateFaceDirection(
    ...
    if (s.faceRoll != null && s.faceRoll!.abs() > rollThreshold) {
      return const CoachingResult(
  ...
  CoachingResult? _evaluateSelfie(PortraitSceneState s, _PoseIntent intent) {
    ...
    if (horizontalFaceCrop || verticalFaceCrop) {
      final message = horizontalFaceCrop && faceCroppedTop
          ? '얼굴이 프레임 밖으로 잘려요'
          : horizontalFaceCrop
          ? '얼굴 옆부분이 잘려요'
          : '이마 쪽이 잘려요';
