...
''';
  ...
  Future<void> _runBenchmark() async {
    ...
    for (int i = 1; i <= iterations; i++) {
      ...
      debugPrint(
        '[AcutPerf] Benchmark iter $i: total=${iterSw.elapsedMilliseconds}ms',
      );
    }
    ...
    setState(() {
      _loading = false;
      _statusMessage =
          '벤치마크 완료: 총 ${sw.elapsedMilliseconds}ms\nAvg: ${avgTotal.toStringAsFixed(1)}ms, P50: ${p50}ms\nSlowest: $slowest\nBottleneck: Explanation generation must be skipped during batch scoring.';
    });

    debugPrint(
      '[AcutPerf] Benchmark Summary: total=${sw.elapsedMilliseconds}ms avg=${avgTotal}ms p50=${p50}ms slowest=$slowest',
    );
  }
